...
namespace WendigoCharacter
{
    public class AnimationEventControllerComponent : MonoBehaviour
    {
        ...
        private IShakeManager shakeManager;
        ...
        public void ShakeTheEarth()
        {
            ...
            {
                float dist = Vector3.Distance(transform.position, data.Target.Position);
                ScreenShake.Create().withTime(0.3f).WithAxis(1f, 1f, 0f).WithStrength(0.25f, 2f).WithCurve(4f, 0.1f, 0.25f).WithAttenuation(dist, 200f).Launch(shakeManager);
            }
        }
